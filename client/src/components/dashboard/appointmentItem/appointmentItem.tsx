...
const AppointmentItem: React.FC<AppointmentItemProps> = ({ name, location, image, time, status, isToday }) => {
  return (
    <div className="py-6 border-b border-[#e2daf5] last:border-0 duration-300 group px-2">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {isToday && (
            <span className="px-3 py-1.5 bg-[#ffeded] text-[#ef5d82] text-[11px] font-black uppercase tracking-widest rounded-full">
              Today
            </span>
          )}
          <div className="px-4 py-2 bg-[#f3f0ff] rounded-full text-primary font-bold text-sm font-dm">
            {time}
          </div>
          <span className="px-4 py-2 bg-[#ecfdf5] text-[#10b981] rounded-full text-sm font-bold font-dm">
            {status}
          </span>
        </div>
      </div>
